...
def select_and_copy_sub_genome_old(genome, sub_genome, mutation_probability):
...
def select_and_copy_sub_genome(genome, sub_genome, mutation_probability, depth=0, max_depth=100):
    if depth > max_depth:
        return False  # Exceeded maximum recursion depth, return False or a default value

    if isinstance(sub_genome, tuple) and len(sub_genome) >= 3:  # Assuming a valid subgenome is identified by these conditions
        # Perform the intended operation if mutation probability condition is met
        return True  # or the modified subgenome

    elif isinstance(sub_genome, list):
        for sg in sub_genome:
            # Recursively call with increased depth
            result = select_and_copy_sub_genome(genome, sg, mutation_probability, depth + 1, max_depth)
            if result:
                return result

    return False  # Base case: return False if no more recursive calls can be made

def select_and_move_sub_genome(genome, sub_genome, mutation_probability):
    """
    Walks to a leaf gene and 
    """
    if sub_genome is None:
        print("Error: sub_genome is None.")
        return False
    ...
    if mutated == False:
        ...
        if random_variable < mutation_probability:
            ...
            sub_genome_list = list(sub_genome)
            sub_genome_list.remove(sub_sub_genome)
            # Convert back to tuple if necessary
            sub_genome = tuple(sub_genome_list)
