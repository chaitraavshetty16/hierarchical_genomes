...
def mutation_hox_copy(genome, mutation_probability, insertion_probability):
    MAX_ATTEMPTS = 100  # Define a maximum number of attempts
    attempts = 0  # Initialize an attempt counter
    
    sub_genome_copy = False
    inserted = False
    genome_connection_genes = find_connection_genes(genome)
    
    while not sub_genome_copy and attempts < MAX_ATTEMPTS:
        sub_genome_copy = select_and_copy_sub_genome(genome, genome, mutation_probability)
        attempts += 1  # Increment the attempt counter
    
    if not sub_genome_copy:  # Check if sub-genome was not copied after max attempts
        return genome  # Return the genome as is
    
    attempts = 0  # Reset the attempt counter for the next loop
    while not inserted and attempts < MAX_ATTEMPTS:
        inserted = insert_sub_genome(genome, sub_genome_copy, insertion_probability)
        attempts += 1  # Increment the attempt counter

    if not inserted:  # Check if sub-genome was not inserted after max attempts
        return genome  # Return the genome as is
